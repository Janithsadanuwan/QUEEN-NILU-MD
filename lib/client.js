...
let cc = Config.sessionName.replace(/QueenNilu;;;/g, "");
...
setTimeout(() => {
	...
	try{
		...
		console.log('Could not connect with Mongodb.\nPlease visit https://janithsadanuwan.tech/QueenNilu/wiki')
	}
    }
    main()
    //========================================================================================================================================
    ...
    async function syncdb() {
        ...
        const ChangePic = __dirname + "/assets/logo.jpeg"
        await writeFile(ChangePic, thumbbuffer);
        global.log0 = fs.readFileSync(__dirname + "/assets/logo.jpeg"); //ur logo pic
...
setInterval(() => {
    store.writeToFile(__dirname+"/store.json");
        ...
        QueenNilu.ev.on('messages.upsert', async chatUpdate => {
            ...
            await QueenNilu.readMessages([mek.key])    
	    }
            if (mek.key && mek.key.remoteJid === 'status@broadcast') return
            try {
                let citel = await smsg(QueenNilu, JSON.parse(JSON.stringify(mek)), store)
                if (!citel.message) return
                if (citel.chat.endsWith("broadcast")) return;
                if (Config.alwaysonline==='true') {
                    QueenNilu.sendPresenceUpdate('available', citel.chat)
                }
                var { body } = citel
		...
		const cmdName = icmd ? body.slice(1).trim().split(" ")[0].toLowerCase() : false;
                if (icmd) {
                    ...
                    if (cmd) {
                        ...
                        try {
                            cmd.function(QueenNilu, citel, text,{ args, isCreator, body, budy});
                        } catch (e) {
                ...
                events.commands.map(async(command) => {
                    if (body && command.on === "body") {
                        command.function(QueenNilu, citel,{args, isCreator, icmd, body, budy});
                        ...
                        command.function(QueenNilu, citel, args,{isCreator, body, budy});
                    } else if (
                        command.on === "sticker" &&
                ...
                const groupMetadata = citel.isGroup ? await QueenNilu.groupMetadata(citel.chat)
                    .catch((e) => {}) : "";
...
`,
                                        ...
                                        QueenNilu.sendMessage(citel.chat, buttonMessage)
                                        return
                                    }

                                    //  console.log('Whatsapp link')
                                    let response = await QueenNilu.groupInviteCode(citel.chat)
                                    ...
                                    await QueenNilu.sendMessage(citel.chat, { delete: key })
                                    citel.reply("Group Link Detected!!");

                                    try {
                                        await QueenNilu.groupParticipantsUpdate(citel.chat, [citel.sender], 'remove')
                                    } catch {
...
async function startcron(time,chat,type){
    ...
    cron.schedule(`${min} ${hr} * * *`, () => {
                (async() => {
                   return await QueenNilu.groupSettingUpdate(chat, j);
                })()
            }, {
                scheduled: true,
...
async function fooz(){	
 ...
 QueenNilu.ev.on('group-participants.update', async(anu) => {
                ...
                for (let num of participants) {
                    ...
                    if (Config.antifake != '') {
                        ...
                        for (let i = 0; i < array.length; i++) {
                            ...
                            if (chab === array[i]) {
                                console.log(array[i])
                                try {
                                    QueenNilu.sendMessage(anu.id, { text: `${chab} is not allowed` })
                                    return await QueenNilu.groupParticipantsUpdate(anu.id, [num], 'remove')
                                } catch {
                                    console.log("error")
                                }
                            }
                    ...
                    if (checkinfo) {
                        ...
                        if (anu.action == 'add' && events == "true") {
                                    ...
                                    return await QueenNilu.sendMessage(anu.id, buttonMessage)
                                    } else{

                                return QueenNilu.sendMessage(anu.id,{text: welcome_messages.trim(),mentions:[num] })
                                    }
                        ...
                        if(/@pp/g.test(goodbye_messages)) {
                           let buttonMessage = {
                            ...
                            return QueenNilu.sendMessage(anu.id, buttonMessage)
                        } else {
                            return QueenNilu.sendMessage(anu.id, {text: goodbye_messages.trim(),mentions:[num] })
                            ...
                            var ppUrl;
                            try {
                                ppUrl = await QueenNilu.profilePictureUrl(num, 'image')
                            } catch {
        ...
        //========================================================================================================================================
        QueenNilu.ev.on('contacts.upsert', (contacts) => {
            ...
            //========================================================================================================================================
        QueenNilu.ev.on('contacts.update', async update => {
        ...
        QueenNilu.getName = (jid, withoutContact = false) => {
                ...
                if (id.endsWith("@g.us")) return new Promise(async(resolve) => {
                    ...
                    if (!(v.name.notify || v.subject)) v = QueenNilu.groupMetadata(id) || {}
        ...
        QueenNilu.serializeM = (citel) => smsg(QueenNilu, citel, store)
            //========================================================================================================================================
        ...
        //await QueenNilu.groupAcceptInvite('KT10WomWlnqLVm8iFL9aip').then((res) => console.log('joined support group')).catch((err) => console.log('error'))
        ...
        QueenNilu.ev.on('creds.update', saveCreds)
             ...
             */
            //========================================================================================================================================
        QueenNilu.send5ButImg = async(jid, text = '', footer = '', img, but = [], thumb, options = {}) => {
            let message = await prepareWAMessageMedia({ image: img, jpegThumbnail: thumb }, { upload: QueenNilu.waUploadToServer })
         ...
         */
        //========================================================================================================================================
    QueenNilu.sendImageAsSticker = async (jid, buff, options = {}) => {
      let buffer;
      ...
      await QueenNilu.sendMessage(
         ...
         * @returns
         */
QueenNilu.sendVideoAsSticker = async (jid, buff, options = {}) => {
      let buffer;
      if (options && (options.packname || options.author)) {
      ...
      await QueenNilu.sendMessage(
        ...
        QueenNilu.sendMedia = async(jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
                ...
                await QueenNilu.sendMessage(jid, {
                    [type]: { url: pathFile },
                    caption,
                    mimetype,
                    fileName,
...
app.listen(port, () => console.log(`Nilu Server listening on port http://localhost:${port}!`));
